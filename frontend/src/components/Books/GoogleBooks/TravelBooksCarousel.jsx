...
const TravelBooksCarousel = () => {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        const response = await fetch('https://www.googleapis.com/books/v1/volumes?q=subject:travel&maxResults=40');
        const data = await response.json();
        setBooks(data.items || []);
      } catch (error) {
        console.error('Error al buscar libros de Viaje:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '300px' }}>
        ...
        <Typography variant="h6" color="textSecondary">Cargando libros de Viaje...</Typography>
      </Box>
    );
  }
...
export default TravelBooksCarousel;
