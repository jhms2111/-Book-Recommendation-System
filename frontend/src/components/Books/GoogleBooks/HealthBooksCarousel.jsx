...
const HealthBooksCarousel = () => {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        const response = await fetch('https://openlibrary.org/search.json?q=health');
        const data = await response.json();
        setOpenLibraryBooks(data.docs || []);
      } catch (error) {
        console.error('Error al buscar libros de Salud:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '300px' }}>
        ...
        <Typography variant="h6" color="textSecondary">Cargando libros de Salud...</Typography>
      </Box>
    );
  }
...
export default HealthBooksCarousel;
